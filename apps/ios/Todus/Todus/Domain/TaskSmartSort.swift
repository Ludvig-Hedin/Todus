...
///
/// Within "due today" and "no due-date, high priority", priority breaks ties.
enum TaskSmartSort {
    /// Visible bucket the smart sort exposes to the UI. The list view
    /// renders one section per non-empty bucket so the user sees structure
    /// ("Today / This week / Later / No date") instead of a flat scroll.
    enum Bucket: Int, CaseIterable, Identifiable {
        ...
        /// One-liner tucked under the header to remind the user what the
        /// section is for. Kept short — a header subtitle, not a paragraph.
        var subtitle: String {
            switch self {
    ...
    /// Buckets a task list into ordered sections preserving smart-sort
    /// ranking inside each bucket. Empty buckets are skipped.
    static func bucketed(
        _ tasks: [TaskRecord],
        now: Date = .now
    ) -> [(bucket: Bucket, tasks: [TaskRecord])] {
    ...
    private static func score(for task: TaskRecord, now: Date, calendar: Calendar) -> Score {
        ...
        guard let due = task.dueDate else {
            // No due date — high priority floats to its own bucket above the no-date pool.
        ...
        if due < now && !calendar.isDateInToday(due) {
            // Overdue: most overdue (oldest date) surfaces first.
