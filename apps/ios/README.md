...
The native app styling is intentionally aligned to the web app in `apps/mail/`.

- Shared semantic colors still come from `@zero/design-tokens`.
- Native screens derive a denser surface system from those web tokens in `src/shared/theme/ThemeContext.tsx`.
- The iOS shell now favors compact typography, soft monochrome surfaces, restrained accent usage, and card-based grouping instead of flat full-width rows.
