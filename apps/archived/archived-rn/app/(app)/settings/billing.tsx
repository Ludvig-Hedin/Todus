  ...
  const billingPortalMutation = useMutation({
    ...
    onSuccess: async (url) => {
      ...
      try {
        ...
        console.error('[billing] failed to open billing portal URL', error);
        captureEvent('billing_portal_open_error', {
          source: 'native_settings_billing',
          error: error?.message ?? String(error),
        });
        Alert.alert(
          'Could not open billing portal',
          'Please open billing in the web app or try again.',
        );
      }
    },
  ...
  const upgradeMutation = useMutation({
    ...
    onSuccess: async (url) => {
      ...
      try {
        ...
        console.error('[billing] failed to open checkout URL', error);
        captureEvent('billing_upgrade_open_error', {
          source: 'native_settings_billing',
          error: error?.message ?? String(error),
        });
        Alert.alert(
          'Could not open checkout',
          'Please try again or continue in the web app.',
        );
      }
    },
    onError: (error: any) => {
