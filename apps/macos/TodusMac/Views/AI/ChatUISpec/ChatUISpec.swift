...
struct ChatUISpecParser {
    /// Regex pattern matching ```ui-spec JSON blocks. Pattern is a hardcoded literal — the
    /// `try?` fallback to an empty regex would silently match every position, so we force-try
    /// to surface a misuse during development instead of producing nonsensical results in prod.
