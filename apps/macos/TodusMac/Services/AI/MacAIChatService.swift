...
final class MacAIChatService {
    ...
    private func executeMemoryTool(named name: String, argsData: Data) async -> String {
        do {
            ...
            let data = try await apiClient.sendRawData(path: "api/ai/do/\(name)", body: body)
            return String(data: data, encoding: .utf8)
                ?? Self.encodeToolResult(success: false, message: "Unreadable memory result")
        } catch {
            return Self.encodeToolResult(success: false, message: "Memory lookup failed: \(error.localizedDescription)")
        }
    }
