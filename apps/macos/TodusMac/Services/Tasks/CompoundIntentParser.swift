...
struct CompoundIntentParser {
    ...
    static func parse(
        ...
        guard segments.count > 1 else {
            // Single segment — classify and parse date
            let parsed = LocalTaskParsingService.parseImmediate(
                rawText: trimmed, now: now, locale: locale, timeZone: timeZone
            )
            return [ParsedIntent(
                type: classifyIntent(trimmed, locale: locale),
                title: parsed.title,
                date: parsed.dueDate
            )]
        }
    ...
    private static func classifyIntent(_ text: String, locale: Locale) -> IntentType {
        ...
        let eventKeywords = [
            ...
            // English
            "meet", "meeting", "lunch", "dinner", "breakfast",
            "coffee", "date", "session", "conference", "interview",
        ]
