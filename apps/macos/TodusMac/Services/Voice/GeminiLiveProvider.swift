...
final class GeminiLiveProvider: VoiceProvider, @unchecked Sendable {
    ...
    private var didReachSetupComplete = false
    ...
    private func scheduleSetupTimeout() {
        ...
        setupTimeoutTask = Task { [weak self] in
            ...
            guard let self, !Task.isCancelled, !self.didReachSetupComplete else { return }
            ...
            while !Task.isCancelled {
                ...
                do {
                    ...
                    if !Task.isCancelled {
                        ...
                        // If we tear down before setupComplete, kill the timeout task too — otherwise
                        // it can fire later and emit a duplicate error after we've already surfaced one.
