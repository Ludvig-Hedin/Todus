...
final class AudioPlayerManager: @unchecked Sendable {
    ...
    private func ensureEngineRunning() {
        ...
        do {
            ...
            // instead of hitting "node is already attached" errors.
            engine.stop()
    ...
    private func schedulePCMBuffer(_ data: Data) {
        // Each frame contains one Int16 sample per channel — divide by channelCount so a future
        // stereo (or higher) configuration doesn't end up with double-counted frames.
        let bytesPerFrame = Int(outputFormat.channelCount) * MemoryLayout<Int16>.size
