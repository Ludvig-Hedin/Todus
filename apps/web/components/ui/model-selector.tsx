...
type AIProvider = keyof typeof PROVIDER_LABELS;
...
export function ModelSelector({ className, variant = 'compact', onModelChange }: ModelSelectorProps) {
  ...
  const [provider, setProvider] = useState<AIProvider>(
    (settings?.settings?.aiProvider as AIProvider | undefined) ?? 'auto',
  );
      ...
      <div className={cn('flex items-center gap-1', className)}>
        ...
        <Select
          ...
          onValueChange={(value) => {
            ...
            const nextProvider = newProvider as AIProvider;
            if (nextProvider !== provider) {
              setProvider(nextProvider);
              setModelId(newModel);
              saveSettings.mutate({ aiProvider: nextProvider as any, aiModel: newModel });
              onModelChange?.(nextProvider, newModel);
            } else {
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      ...
      <div className="space-y-1.5">
        <label id="ai-provider-label" className="text-sm font-medium">
          AI Provider
        </label>
        <Select value={provider} onValueChange={handleProviderChange}>
          <SelectTrigger
            id="ai-provider-select"
            aria-labelledby="ai-provider-label ai-provider-select"
            className="w-full"
          >
      ...
      {provider !== 'auto' && (
        <div className="space-y-1.5">
          <label id="ai-model-label" className="text-sm font-medium">
            Model
          </label>
          <Select value={modelId} onValueChange={handleModelChange}>
            <SelectTrigger
              id="ai-model-select"
              aria-labelledby="ai-model-label ai-model-select"
              className="w-full"
            >
            ...
            <SelectContent>
              {provider === 'openrouter' ? (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  Select a model from the dropdown when available, or use a saved model ID such as
