...
1. **MAC-1** — TodusMacTests can't run in Xcode. **Attempted a real fix this pass** (standalone host-less test bundle: extracted `GetThreadResponse`/`FailableDecodable` to a Foundation-only `Domain/EmailThreadResponse.swift`, pointed the test target at `EmailModels.swift` + that file, dropped `@testable`/host dep, ran `xcodegen generate`). It **failed for a concrete reason**: `EmailModels.swift` itself depends on `AppLogger` (an app type), which cascades back toward the MLX-linked app, AND the test scheme still builds the `TodusMac` target whose `Cmlx`/`_NumericsShims` C modules don't resolve in the test build context. A real fix therefore needs either (a) extracting `EmailModels` + `AppLogger` + their transitive deps into a separate MLX-free framework/SPM target, or (b) fixing the MLX C-module search paths for the testable host build — both larger structural changes needing iterative `xcodebuild test` cycles and a human-reviewed pbxproj regen. **Cleanly reverted** (cp backups) — macOS app + tree are back to green, nothing left broken. The exact coverage already runs via `scripts/run-email-decode-tests.sh` (11/11). **→ deliberate human structural fix.**
