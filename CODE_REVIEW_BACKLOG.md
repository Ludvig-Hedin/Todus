...
|----|------|-------|---------|---------------|
| PAR-A2 | Calendar | `apps/web/app/(routes)/mail/calendar/page.tsx`, `apps/web/app/(routes)/settings/calendars/page.tsx`, `apps/server/src/trpc/routes/calendar.ts` | Multi-calendar visibility toggles not built; page hardcodes `primary`. Write mutations are `activeConnectionProcedure` (no `connectionId`) while `eventsMulti` spans connections — asymmetry blocks non-primary/multi-connection editing. | Add a `calendarsMulti` query + optional `connectionId` on `createEvent/updateEvent/deleteEvent`; build the left-rail calendar list + localStorage visibility store; wire `settings/calendars` to it. |
| PAR-B2 | AI tools | `apps/server/src/routes/agent/tools.ts` | AI cannot create calendar events (only tasks, as of B1). `aiCanWriteCalendar` setting saved but not enforced. | Add a `createEvent` agent tool reusing `calendar.createEvent`'s Google client; gate behind `aiCanWriteCalendar`. |
...
| PAR-B-TEST | AI tools | `apps/server/src/routes/agent/tools.ts` | New task tools are DB-backed; no automated test (server suite has no DB harness). Verified via tsc + server test suite (no import/compile breakage). | Add an integration test once a DB test harness exists; for now manual verify via chat. |
