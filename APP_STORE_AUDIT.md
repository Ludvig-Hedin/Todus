...
disclosure before chat/voice sends data, iOS no longer compiles the native public AI share-link or group
chat surfaces, and the simulator build/test suite was green after the first audit pass.
...
- iOS flows: auth, onboarding, settings, billing, account deletion, permissions, AI, docs, calendar,
...
- 2026-06-19 UGC target pruning: removed the now-unreachable native public AI share-link views/services
  and group chat views/services from the iOS target so they are not compiled into the submission build.
...
- 2026-06-19 post-UGC-target-pruning validation: fallback `xcodebuild build` and
  `xcodebuild test -quiet` passed with `CODE_SIGNING_ALLOWED=NO`. Build output confirmed stale
  group/share object files were removed from DerivedData.
...
| User-generated content / sharing            | PASS/PARTIAL                            | iOS AI toolbar sharing now uses the local system share sheet with a redacted transcript instead of creating hosted public links. `todus://share` no longer opens public shared conversations in iOS. Native public share-link and group-chat views/services were removed from the iOS target. Web/server sharing still exists outside this iOS submission surface. | Low      | Keep group/public sharing out of the iOS submission build, state this in review notes if needed, and add report/block/moderation/contact flows before shipping public or social UGC in iOS.                  |
...
6. Keep UGC/group/share features out of the iOS submission surface.
   ...
   and group chat views/services from the iOS target. Do not re-enable public/social UGC in iOS until
   report, block, moderation, and contact workflows satisfy Guideline 1.2.
...
- Removed unreachable native public share-link and group-chat views/services from the iOS target.
