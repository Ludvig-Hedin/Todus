...
- `DONE` **Verification status for the native voice fix (2026-04):** iOS `xcodebuild -project apps/ios/Todus/Todus.xcodeproj -scheme Todus -configuration Debug -destination 'generic/platform=iOS' build` succeeded after the patch. macOS full-build verification is currently blocked by an unrelated pre-existing compile failure in `apps/macos/TodusMac/App/TodusMacApp.swift` because `FolderItemRecord` is not in scope on this branch.
