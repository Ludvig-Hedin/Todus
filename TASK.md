...
- `DONE` **Calendar (macOS)**: `MacCalendarView` month mode uses a vertically paged month stack (`scrollPosition` + `viewAligned`); horizontal trackpad steps day/week/month/year via `CalendarTrackpadNavigation` with a shared spring animation; `loadEvents` for Month covers prev/current/next month; trackpad `PreferenceKey` + callback sync use `Equatable` tokens (Swift 6 build clean).
