...
3. **No project-wide lint/format.** `pnpm check` / `pnpm lint` / `pnpm format` sweep the whole monorepo. Only lint/format files you touched (`npx eslint <file>`, `npx prettier --write <file>`).
...
- **Deploy the active app with `pnpm --filter=@zero/web build|deploy`.** Root `build:frontend`/`deploy:frontend` still ship the legacy `@zero/mail`.
...
- **`pnpm dev` runs `apps/web` on `:3000`** and proxies `/api`,`/sse`,`/agents` to the backend on `:8787`.
